...
if (!function_exists('getAllChampsFromDB')) {
	function getAllChampsFromDB($db) {
		$sql = "SELECT chpidchp, chplbcde, chplbnom, chpfgnul, chpcdtyp, chplbdes, objidobj from spechp ";
		$query = $db->query($sql);
...
if (!function_exists('insertNewChamp')) {
	function insertNewChamp($db, $chplbcde, $chplbnom, $chpfgnul, $chpcdtyp, $chplbdes, $objidobj) {
		$data=array( 'chplbcde'=>$chplbcde, 'chplbnom'=>$chplbnom, 'chpfgnul'=>$chpfgnul, 'chpcdtyp'=>$chpcdtyp, 'chplbdes'=>$chplbdes, 'objidobj'=>$objidobj );
...
if (!function_exists('updateChamp')) {
	function updateChamp($db, $chpidchp, $chplbcde, $chplbnom, $chpfgnul, $chpcdtyp, $chplbdes, $objidobj) {
		$sql = "update spechp set chplbcde = ?, chplbnom = ?, chpfgnul = ?, chpcdtyp = ?, chplbdes = ?, objidobj = ? where chpidchp = ?";
		$query = $db->query($sql, array($chplbcde, $chplbnom, $chpfgnul, $chpcdtyp, $chplbdes, $objidobj, $chpidchp));
	}
}
